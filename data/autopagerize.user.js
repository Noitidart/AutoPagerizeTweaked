...
AutoPager.prototype.getNextURL = function(xpath, doc, url) {
    ...
    if (nextLink) {
        ...
        if (!nextValue || !nextValue.match(/^http(s)?:/)) {
            var base = getFirstElementByXPath('//base[@href]', doc)
            nextValue = resolvePath(nextValue, (base ? base.href : url))
        }
        if (nextValue && isSameDomain(nextValue)) {
            return nextValue
        }
    }
}
